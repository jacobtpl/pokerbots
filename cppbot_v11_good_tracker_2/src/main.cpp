...
struct Bot {
	...
	double getBoardStrength(pair<int,int> hole, pair<int,int> oppHole, vector<int> board) {
		HandEvaluator eval;
	...
	vector<int> prevBoard(vector<int> board) {
		if (board.size() == 3) {
			return vector<int>();
		}
		vector<int> b2;
		for (int i=0;i<board.size()-1;i++) b2.push_back(board[i]);
		return b2;
	}
