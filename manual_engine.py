    ...
    def query(self, round_state, player_message, game_log):
        ...
        if self.socketfile is not None and self.game_clock > 0.:
            ...
            try:
                ...
                print(self.name + ' attempted illegal ' + action.__name__)
            except socket.timeout:
                ...
                print(self.name + ' response misformatted: ' + str(clause))
...
class Game():
    ...
    def log_round_state(self, players, round_state):
        ...
        if round_state.street == 0 and round_state.button == 0:
            self.good_append('{} posts the blind of {}'.format(players[0].name, SMALL_BLIND))
            self.good_append('{} posts the blind of {}'.format(players[1].name, BIG_BLIND))
            if self.show_opp_hands or players[0].name == PLAYER_2_NAME:
                self.good_append('{} dealt {}'.format(players[0].name, PCARDS(round_state.hands[0])))
            if self.show_opp_hands or players[1].name == PLAYER_2_NAME:
                self.good_append('{} dealt {}'.format(players[1].name, PCARDS(round_state.hands[1])))
            ...
            self.good_append(STREET_NAMES[round_state.street - 3] + ' ' + PCARDS(board) +
            ...
            if round_state.street < 5:
                if self.show_opp_hands or players[0].name == PLAYER_2_NAME:
                    self.good_append("{}'s hand: {}".format(players[0].name, PCARDS(round_state.hands[0])))
                if self.show_opp_hands or players[1].name == PLAYER_2_NAME:
                    self.good_append("{}'s hand: {}".format(players[1].name, PCARDS(round_state.hands[1])))
    ...
    def log_action(self, name, action, bet_override):
        ...
        self.good_append(name + phrasing)
    ...
    def log_terminal_state(self, players, round_state):
        ...
        self.good_append('{} shows {}'.format(players[0].name, PCARDS(previous_state.hands[0])))
        self.good_append('{} shows {}'.format(players[1].name, PCARDS(previous_state.hands[1])))
        ...
        self.good_append('{} awarded {}'.format(players[0].name, round_state.deltas[0]))
        self.good_append('{} awarded {}'.format(players[1].name, round_state.deltas[1]))
    ...
    def run(self):
        ...
        soh = input('Show opponent hands? [y/n]: ')
        if soh == 'y':
            self.show_opp_hands = True
        elif soh == 'n':
            self.show_opp_hands = False
        else:
        ...
        for round_num in range(1, NUM_ROUNDS + 1):
            self.good_append('')
            self.good_append('Round #' + str(round_num) + STATUS(players))
            self.run_round(players)
            players = players[::-1]
        self.good_append('')
        self.good_append('Final' + STATUS(players))
        for player in players:
            player.stop()
