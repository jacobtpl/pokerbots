...
struct PreflopTracker {
	...
	double get_nit_factor() {
		if (counts[0][0][2]+counts[0][1][2] == 0) return 1.0;
		if (counts[1][0][2]+counts[1][1][2] == 0) return 1.0;

		double open0 = (double)(stats[0][0][2]+stats[0][1][2]) / (double)(counts[0][0][2]+counts[0][1][2]);
		double open1 = (double)(stats[1][0][2]+stats[1][1][2]) / (double)(counts[1][0][2]+counts[1][1][2]);
		return open0 / open1;
	}
...
struct Bot {
	...
	*/
    Action getAction(GameInfoPtr gameState, RoundStatePtr roundState, int active) {
		...
		if (roundNum > 100) {
			ratio /= min(tracker.get_nit_factor(), 2.0);
		}
		int raiseAmount = 0;
		if (street < 3) {
			...
			if (roundNum > 100) {
				raiseAmount = (int)((double)raiseAmount / min(tracker.get_nit_factor(), 2.0));
			}
		} else {
		...
		printf("Nit factor: %lf\n", tracker.get_nit_factor());
		// if (roundNum > 100) {
		// 	scared_strength /= tracker.get_nit_factor();
		// }
