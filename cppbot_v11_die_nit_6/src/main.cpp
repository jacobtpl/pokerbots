...
struct PreflopTracker {
	...
	double get_stat(int player, int blind, int idx) {
		if (counts[player][blind][idx] == 0) {
			return 0.0;
		}
    ...
    string get_stat_str(int player, int blind) {
		stringstream out;
		for (int i=0;i<5;i++) {
			string x = NAMES[i];
...
struct Bot {
	...
	void setPreflopConstants(int nit) {
		// nit from -2 to 2
		open_cutoff = 80 + nit * 10;
		open_defend = 44 - nit * 8;
		open_reraise = 15 - nit * 4;
		open_redefend = 12 - nit * 4;
	...
	*/
    void handleRoundOver(GameInfoPtr gameState, TerminalStatePtr terminalState, int active) {
		...
		cout << "Us in SB: " << tracker.get_stat_str(0, 0) << endl;
		cout << "Us in BB: " << tracker.get_stat_str(0, 1) << endl;
		cout << "Op in SB: " << tracker.get_stat_str(1, 0) << endl;
		cout << "Op in BB: " << tracker.get_stat_str(1, 1) << endl;
		cout << endl;
	}
			...
			if (oppContribution <= 2) {
				...
				if (bigBlind) {
					...
					if (roundNum > round_start_using_tracker) {
						...
						setPreflopConstants(0);
						if (pct >= 0.2 && pct < 0.3) {
							ratio = 4.0;
							setPreflopConstants(-1);
				...
				raiseAmount = 4 * oppContribution;
			} else if (oppContribution <= 60) {
				raiseAmount = 11 * oppContribution / 5;
			} else {
				raiseAmount = 200;
			}
		} else {
