...
class Player(Bot):
    ...
    def calc_strength(self, hole, iters, board):
        ...
        for opp_hole_s, weight in self.weights.items():
            opp_hole = [eval7.Card(c) for c in opp_hole_s.split()]

            if opp_hole[0] in our_hand or opp_hole[1] in our_hand:
                continue
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        for i in range(len(deck)):
            for j in range(i+1, len(deck)):
                self.weights[to_str([deck[i], deck[j]])] = 1.0
