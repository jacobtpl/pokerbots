...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        percentile = (percentile + strength)/2
        aggressive = True
        if continue_cost > 0: 
            pot_odds = continue_cost/(pot_total + continue_cost)
            if percentile >= pot_odds: # nonnegative EV decision
                if percentile > 0.8 and random.random() < percentile: 
            ...
            if percentile > 0.3 and random.random() < percentile: 
                my_action = temp_action
            else: 
        ...
        if aggressive:
            cutoff = range_strength[len(range_strength)//4]
            out_of_range = []
            for hand in self.range:
